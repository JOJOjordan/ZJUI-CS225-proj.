...
// return the minimum key value in the tree
KeyType CBPlusTree::minimum() {
    if (m_Root == nullptr) {
        cout << "Empty tree";
        return 0;
    }
    return m_DataHead->getKeyValue(0);
}

// return the maximum key value in the tree
KeyType CBPlusTree::maximum() {
    if (m_Root == nullptr) {
        cout << "Empty tree";
        return 0;
    }
...
// return the predecessor key value of the given key
KeyType CBPlusTree::predecessor(KeyType key) {
    CLeafNode *node = position_key(key);
    if (node == nullptr) {
        cout << "No such key";
        return 0;
    }
    ...
    int index;
    for (index=0; index < node->getKeyNum(); ++index)
    {
        if (node->getKeyValue(index) == key) {
            break;
        }
    }
    ...
    if (index == 0) {
        ...
        // search the last key in the previous node
        node = node->getLeftSibling();
        return node->getKeyValue(node->getKeyNum() - 1);
    }
    
    else { // index > 0
        return node->getKeyValue(index - 1);
    }
    
}

// return the successor key value of the given key
KeyType CBPlusTree::successor(KeyType key) {
    CLeafNode *node = position_key(key);
    ...
    if (index == node->getKeyNum() - 1) {
        ...
        // search the first key in the next node
        node = node->getRightSibling();
        return node->getKeyValue(0);
    }
    
    else { // index < number of keys - 1
        return node->getKeyValue(index + 1);
    }
        
}

CLeafNode *CBPlusTree::position_key(KeyType key) {
...
void CBPlusTree::recursive_remove(CNode* parentNode, KeyType key)
{
    ...
    else // 内结点
    {
        ...
        {
            CNode *pLeft = childIndex>0 ? ((CInternalNode*)parentNode)->getChild(childIndex-1) : nullptr;                       //左兄弟节点
            CNode *pRight = childIndex<parentNode->getKeyNum() ? ((CInternalNode*)parentNode)->getChild(childIndex+1) : nullptr;//右兄弟节点
            // 先考虑从兄弟结点中借
...
vector<DataType> CBPlusTree::select(KeyType compareKey, int compareOpeartor)
{
    ...
    if (m_Root!=nullptr)
    {
        if (compareKey>m_MaxKey)   // 比较键值大于B+树中最大的键值
        {
            ...
            {
                for(CLeafNode* itr = m_DataHead; itr!=nullptr; itr= itr->getRightSibling())
                {
                    for (int i=0; i<itr->getKeyNum(); ++i)
                    {
