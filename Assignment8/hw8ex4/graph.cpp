...
template<class T> int vertexlist<T>::removeid(int num)
{
    ...
    if ((*pt).getedges() != 0)
        {
            cout << "The vertex "  << " cannot be deleted, as it appears in an edge.\n";
            return (*pt).getitem();
        }
    location = pt;
    /* Finally, we get to the deletion from the veertexlist. */
...
template<class T> int vertexlist<T>::getit(int num)
{
    vertex<T> *pt = (*dummy).getnext();
    for (int i = 0; i < num; i++)
    {
...
template<class T> edgelist<T> * vertexlist<T>::incidentedges(T item)
{
    edgelist<T> * result = new edgelist<T>;
    vertex<T> * ptv = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
...
template<class T> void elist<T>::addall2(T item, edgelist<T> *pt)
{
    neighbour<T> * pte = first;
    for (int i = 0; i < length; i++)
    {
        (*pt).add(item, (*pte).getitem());
        (*pt).add2(item, (*pte).getitem());
        pte = (*pte).getnext();
    }
    return;
}
...
template<class T> void edgelist<T>::add2(T item1, T item2)
{
    edge<T> * newedge = new edge<T>(item2,item1);
    reprarray[numedges] = newedge;
    ++numedges;
    return;
...
template<class T> BiPartiteGraph<T>::BiPartiteGraph(void)
{
    vertices = new vertexlist<T>;
    V1= new vertexlist<T>;
    V2= new vertexlist<T>;
...
template<class T> int BiPartiteGraph<T>::numvertices(void)
{
    return (*vertices).getlength();
}

template<class T> int BiPartiteGraph<T>::numedges(void)
{
    return (*vertices).countedges();
}

template<class T> void BiPartiteGraph<T>::insert_edge(T origin, T destination)
{
    if ((V1->contains(origin)== true) &&(V1->contains(destination)== true)){
        cout<<"X:You can't do this because V1 contain these two vertices."<<"\n";
    }
    else if ((V2->contains(origin)== true) &&(V2->contains(destination)== true)){
        cout<<"X:You can't do this because V2 contain these two vertices."<<"\n";
    }
    else if ((V2->contains(origin)== true) &&(V1->contains(destination)== true)){
        cout<<"X:Maybe you should change the places of origin and destination"<<"\n";
    }
    else{                                               //we default add origin to V1 and destination to V2, other case(like add origin to V2) is not allowed.
        (*V2).addvertex(destination);
        (*V1).addvertex(origin);
        (*vertices).addedge(origin, destination);
        (*vertices).addedge(destination, origin);
    }
...
template<class T> void BiPartiteGraph<T>::insert_vertex(T item, int Vx)// add item to V1 or V2.
{
    if (Vx==1) {
        if (V2->contains(item)== true){
            cout<<"You can't add "<<item <<" to V1 because V2 contain it."<<"\n";
        }
        else {
            (*V1).addvertex(item);
            (*vertices).addvertex(item);
    ...
    if (Vx==2) {
        if (V1->contains(item)== true){
            cout<<"You can't add "<<item <<" to V2 because V1 contain it."<<"\n";
        }
        else {
            (*V2).addvertex(item);
            (*vertices).addvertex(item);
...
template<class T> void BiPartiteGraph<T>::delete_edge(T origin, T destination)
{
    if ((V1->contains(origin)== true) && (V2->contains(destination)== true)){
        (*vertices).removeedge(origin, destination);
        (*vertices).removeedge(destination, origin);// as it is an undirected graph, both A to B and B to A should be deleted.
        (*V1).remove(origin);
        (*V2).remove(destination);          
    }                             
    else{
        cout<<"error with V1 not contain "<<origin <<"or V2 not contain "<<destination <<"\n";
    }
    return;
}

template<class T> void BiPartiteGraph<T>::delete_vertex(T item)
{
    (*vertices).remove(item);
    (*V1).remove(item);//delete it from v1.
    (*V2).remove(item);//delete it from v2.
    return;
}
...
template<class T> edgelist<T> * BiPartiteGraph<T>::incident_edges(T item)
{
	/*Implement your code here*/
    return (*vertices).incidentedges(item);             // this is a little different with template outgoingedges as it will push(a,b) and (b,a) as an undirectedgraph.
}

/*determine the perfect match*/

template<class T> void BiPartiteGraph<T>::is_perfect_match(void)    // we use Hall's marriage theorem to determine if it is perfect mactch.
{
    V3 = new vertexlist<T>;
    V4 = new vertexlist<T>;
    V5 = new vertexlist<T>;
    ...
    a3=1;
    if (n1>x/2) a0=0;
    recursion1();// recursion 1 to determine if for all case and all subset the number of edges greater than vertices.
    
    if (n2>x/2) a2=0;
    recursion2();
    if (((a0==1)&&(a1==1))&&((a2==1)&&(a3==1))) {                   // If the number of edges is greater than the number of V1 and V2 in all subcase, it is perfect
        cout<<"have perfect match"<<"\n";
    }
    else cout<<"don't have perfect match"<<"\n";
	return ;
}

template<class T> void BiPartiteGraph<T>::recursion1(void){     //recursion function 1
    int length1= V3->getlength();
...
template<class T> void BiPartiteGraph<T>::recursion2(void){     //similar to above.
    int length1= V5->getlength();
    int length2= V1->getlength();
    for (int i=1; i< length1; i++){
        int a= V5->getit(i);
        for (int j=0;j< length2; j++){
            int b = V1->getit(j);
...
template<class T> void BiPartiteGraph<T>::prettyprint(void)
{
    cout << "\n\n====== Graph ======\n\n";
    (*vertices).prettyprint();
    cout << "=================\n\n";
