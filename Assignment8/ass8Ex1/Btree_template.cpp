...
public:
    ...
    BtreeNode *search_parent(int k);// search the parent of the given key
...
// search the parent of the given key
BtreeNode * BtreeNode :: search_parent(int k)
{    int i;
    for(i=0;i<n && a[i]<k;i++);
    if(a[i] == k)
        return this;
    ...
    // return the parent of the searched node
    if (C[i]->leaf == true && C[i]->search(k) == C[i]) {return this;}
...
int Btree::find_max()
{
    ...
    while (node->leaf != true) {
        node = node->C[node->n]; // search the largest child // ???
    }
...
int Btree::find_min()
{
    ...
    while (node->leaf != true) {
        node = node->C[0]; // search the smallest child
    }
...
int main()
{
    ...
    cout << "The maximum number is : " << t.find_max() << endl;
    cout << "The minimum number is : " << t.find_min() << endl;
