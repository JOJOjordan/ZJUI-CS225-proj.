...
public:
    ...
    int getlength(void);
    int countedges(void);
    void append(T item,int sex = 0);
    void addvertex(T item,int sex = 0);
    // modified there to restore the sex information.
...
public:
    ...
    void prettyprint(void); // only for testing
    // why the edgelist cannot access to the reprearray?
    // so why you define such a reprarray in such a class?
    // just to store but do not access???
    edge<T>** get_reprarray();
    edge<T>* get_edge(int index);
    void append(edgelist<T>* newarray);
private:
    edge<T> ** reprarray;
    int numedges;
};
