...
template<class T> int vertex<T>::get_sex() {
    return this->sex;
}

template<class T> void vertex<T>::set_sex(int sex) {
    this->sex = sex;
    return;
}
...
template<class T> void vertexlist<T>::append(T item,int sex)
{
...
template<class T> void vertexlist<T>::addvertex(T item,int sex)
{
...
// self_made functions
template<class T> edge<T> * edgelist<T>::get_edge(int index) {
    return reprarray[index];
}

template<class T> void edgelist<T>::append(edgelist<T> *newarray) {
    for (int i=0;i<newarray->getnumedges();i++){
        reprarray[numedges]=newarray->get_edge(i);
        numedges++;
    }
    return;
}

template<class T> edge<T> ** edgelist<T>::get_reprarray() {
    return reprarray;
}
