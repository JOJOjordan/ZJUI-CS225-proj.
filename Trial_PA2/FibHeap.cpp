...
// transfer the data from tuple to fib_node
template <class T> void CentralQueue<T>::transfer_in(Tuple *tuple, FibNode<T> *fib_node) {
    fib_node->ID        = tuple->get_id();
    fib_node->name      = tuple->get_name();
    fib_node->Medic     = tuple->get_medic();
    fib_node->Reg       = tuple->get_registration();
    fib_node->stat      = tuple->get_status();
    fib_node->Treatment = tuple->get_Treatment();
    fib_node->Treat_day = tuple->get_TreatDay();
    fib_node->start_day = tuple->get_startDay();
    
    fib_node->priority = tuple->get_priority();
}

// transfer the data from fib_node to tuple
/*template <class T> void CentralQueue<T>::transfer_out(Tuple *tuple, FibNode<T> *fib_node) {
...
// the input is *Tuple. call this function will generate a node in the FibHeap
template <class T> void CentralQueue<T>::record_in(Tuple *tuple) {
    if (tuple == nullptr) {return;}
    
    FibNode<int> *fib_node = new FibNode<int>(0);
    transfer_in(tuple, fib_node);
    
    // for those who ever withdrew, go to waiting_heap and waiting 2 weeks(14 days)
    if (fib_node->start_day != date) {
        // waiting date = Reg_Day + 14
        fib_node->waiting_date = tuple->get_startDay();
...
template <class T> Tuple *CentralQueue<T>::record_out() {
    ...
    Tuple *tuple = new Tuple(fib_node->ID, fib_node->name, fib_node->Medic, fib_node->Reg, fib_node->Treatment, fib_node->stat);
    tuple->set_startDay(fib_node->start_day);
    tuple->set_TreatDay(date+1);
    
    if (fib_node != nullptr) {delete fib_node;} // release the space
    return tuple; // considering ptr == NULL
     
}

// search the node with key and ID in all 4 heaps. return a pointer to the node; if not find, return NULL
template <class T> void CentralQueue<T>::search_node(Tuple *tuple, FibHeap<T> **heap, FibNode<T> **fib_node) {
    FibNode<T> *root_node;
    ...
    if (*fib_node == nullptr) {
        key = tuple->get_priority(); // the priority should not change!!
...
/*template <class T> void CentralQueue<T>::change_profession(Tuple *tuple) {
    FibNode<T> *fib_node = nullptr;
    FibHeap<T> *heap = nullptr;
    // search the node in the heap
    search_node(tuple, &heap, &fib_node);
    // not find
    if (fib_node == nullptr) {return;}
    
    int old_profession = fib_node->profession;
    fib_node->profession = tuple->profession;
    
    tuple->standard_priority = (tuple->profession - 1) * 28 + (tuple->age_group - 1) * 4 +tuple->risk; // ?? does the formula correct? or we donot need "change_profession"
    if (old_profession <= fib_node->profession) {return;}
    fib_node->priority = tuple->standard_priority;
    heap->decrease(fib_node, fib_node->priority);
    return;
}*/

template <class T> void CentralQueue<T>::withdraw_heap(Tuple *tuple) {
    FibNode<T> *fib_node = nullptr;
    FibHeap<T> *heap = nullptr;
    // search the node in the heap
    search_node(tuple, &heap, &fib_node);
...
template <class T> void CentralQueue<T>::build_array(vector<Tuple*> *a, FibNode<T> *root, int n) {
    //int len = waiting_number();
    FibNode<T> *tmp = root;    // temporary node
    //FibNode<T> *p = nullptr;    // target node
    ...
    do
    {
        ...
        Tuple *tuple = new Tuple(root->ID, root->name, root->Medic, root->Reg, root->Treatment, root->stat);
        tuple->set_startDay(root->start_day);
        tuple->set_TreatDay(root->Treat_day);
