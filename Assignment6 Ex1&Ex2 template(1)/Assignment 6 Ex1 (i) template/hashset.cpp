...
template<class T> void hashset<T>::remove_modified(T item) {
    ...
    hash <T> hashfunction;
    //now calc the target index.
    int index = hashfunction(item) % maxsize;
    int start_index = index;
    int distance = 0;
    bool found = false;
    while (reprarray[index] != 0 && !found) {
        if (*reprarray[index] == item) {
            // found the item! but we should continue the search to go the tail of the queue.
            distance = (index - start_index) % maxsize;
            found = true;
        }
        index = (index + 1) % maxsize;
    }
    // now we have let the index go to the tail of the current queue
    int next_index = (index + 1)%maxsize;
    int done = -1;

    while (reprarray[next_index] != 0){
        // find out if there's any element have better distance from the desire position
        // to the removed position.
        int desire = hashfunction(*reprarray[next_index]) - start_index;
        if (desire % maxsize <= distance && done < 0){
            T* temp = reprarray[next_index];
            // recursively move down to make sure that
            // all the element in the queue can be reached.
            remove_modified(*reprarray[next_index]);
            reprarray[(start_index+distance)%maxsize] = temp;
            done = 0;
        }
        next_index = (next_index + 1 )% maxsize;
    }
    // now check if we have finish the replacement before using var done,
    if (done < 0){
        // since we cannot find any possible replacement , we can cut the queue down by here.
        reprarray[(start_index + distance)%maxsize] = 0;
        // true remove happened  there.
        --numitems;
        int load = 100*numitems / maxsize;
        if (load <= 25){
            rehash(maxsize / 2);
